...
It carries only the columns laptops and tablets need - *Asset Tag*, *Type*,
*Status*, *Notes*, *Model/Description*, *Serial number* - in the same order they
appear in the program, already split by room by the separator rows, with
dropdowns on *Type* and *Status* and an *Istruzioni* sheet. The template generated by the program mirrors the rooms and statuses
