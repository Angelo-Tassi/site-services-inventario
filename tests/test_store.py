...
assert vuoto.import_items(items, "replace") == (2, 0)
vuoto.load()
assert len(vuoto.items) == 2
tel = [i for i in vuoto.items if i["asset_tag"] == "351111111111111"][0]
assert tel["stanza"] == BAU, "l'iPhone importato deve finire nella sua stanza"
assert tel["stato"] == "Da Rispedire", tel["stato"]
